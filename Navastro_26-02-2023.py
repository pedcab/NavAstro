...
if obsLimb==0:
    ...
    p2=60*(Ha2-Rc2+PA2-SD2-Hc2)
#CALCULATE APLONG DECIMAL
apLong=apLongDeg+apLongMin/60
apLong2=apLongDeg2+apLongMin2/60 
...
labels = [math.trunc(apLong), ' ', math.trunc(apLong-1)]
plt.xticks(x, labels)
#Axes
...
c=apLong
d=apLongDeg
point9 = [(a-0.5)+((a*c)/(d+0.5)-a),a]
x_values5 = [point9[0]]
y_values5 = [point9[1]]
plt.text(point9[0], point9[1],"  Ap 1",color='blue')
plt.scatter( x_values5 , y_values5 , s=25 ,  facecolors='none', edgecolors='blue' ,linewidth=2 )
#ap2Long point
c2=apLong2
d2=apLongDeg2
point92 = [(a-0.5)+((a*c2)/(d2+0.5)-a),a]
x_values52 = [point92[0]]
y_values52 = [point92[1]]
plt.text(point92[0], point92[1],"  Ap 2",color='magenta')
plt.scatter( x_values52 , y_values52 , s=25 ,  facecolors='none', edgecolors='magenta' ,linewidth=2 )

#Zn1 line
z=(-Zn+90)*DegRad  #VERIFICAR!!!
m=math.tan(z)
b=(a*(d+0.5)*math.cos(z)-(a*c-0.5*(d+0.5))*math.sin(z))/((d+0.5)*math.cos(z))
x_valuesZn1=[((a-1)-b)/m,((a+1)-b)/m]
y_valuesZn1=[a-1, a+1]
plt.plot(x_valuesZn1, y_valuesZn1,linestyle="-",color='blue',linewidth=1)
